...
const PLUGIN_APPS = ['jira', 'sessionstack'];

function getPluginConfig(app) {
  const pyName = app.replace('-', '_');
  const staticPluginPrefix = `src/new_sentry_plugins/${pyName}/static/${pyName}`;
  const distPluginPath = `${staticPluginPrefix}/dist`;

  const config = {
    mode: WEBPACK_MODE,
    name: app,
    entry: `./${pyName}.jsx`,
    context: path.join(__dirname, staticPluginPrefix),
    externals: {
      react: 'React',
      'react-dom': 'ReactDOM',
      'react-router': 'Router',
    ...
    module: {
      rules: [
        {
          test: /\.jsx?$/,
          include: path.join(__dirname, staticPluginPrefix),
    ...
    output: {
      path: path.join(__dirname, distPluginPath),
      filename: `${app}.js`,
...
module.exports = [appConfig, ...PLUGIN_APPS.map(getPluginConfig)];
